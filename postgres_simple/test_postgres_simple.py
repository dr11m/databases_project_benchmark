...
def save_plot_test_result_insert(times, conn_pool):
    ...
    plt.grid(True)
    plt.savefig(f"postgres_simple/results_timescale_without_hypertable/time_to_insert_at_size_{int(total_records / 1000000)}kk_rows.png")  # Сохранить график в файл
...
def save_plot_test_result_select(times, conn_pool):
    ...
    plt.grid(True)
    plt.savefig(f"postgres_simple/results_timescale_without_hypertable/time_to_select_data_for_unique_id_table_size_was_{int(total_records / 1000000)}kk_rows.png")  # Сохранить график в файл

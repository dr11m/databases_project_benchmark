...
def clear_tables(conn_pool):
    ...
    cur.execute("DELETE FROM items_5")
    cur.execute("DELETE FROM items_6")
    cur.execute("DELETE FROM items_7")
    cur.execute("DELETE FROM items_8")
    cur.execute("DELETE FROM items_9")
    cur.execute("DELETE FROM items_10")
    conn.commit()
...
def get_total_records(conn_pool):
    ...
    cur = conn.cursor()
    for i in range(10):
        cur.execute(f"SELECT COUNT(*) FROM items_{i+1}")
...
def read_unique_id_test(amount_of_requests, conn_pool):
    ...
    for i in range(amount_of_requests):
        ...
        print(f"insert: {i}/{amount_of_requests} -- {end_time - start_time}, lenght for all_results: {len(all_results)}")
...
def write_speed_test(config, iteration_amount, conn_pool):
    ...
    for i in range(iteration_amount):
        ...
        start_time = time.time()
        table_num = i % 10 + 1
        write_to_timescaledb(conn_pool, data, table_num)
        end_time = time.time()
        if i % 10 == 0:
            times.append(end_time - start_time)
...
def save_plot_test_result_insert(times, conn_pool):
    ...
    plt.grid(True)
    plt.savefig(f"postgresDB_partitioning_into_10_tables/results_latest_postgres/time_to_insert_at_size_{int(total_records / 1000000)}kk_rows.png")  # Сохранить график в файл
...
def save_plot_test_result_select(times, conn_pool):
    ...
    plt.grid(True)
    plt.savefig(f"postgresDB_partitioning_into_10_tables/results_latest_postgres/time_to_select_data_for_unique_id_table_size_was_{int(total_records / 1000000)}kk_rows.png")  # Сохранить график в файл
...
def run_test_scenario(conn_pool):
    config = {
            ...
            "iterations_at_each_stage": [100, 900, 4000, 5000],
